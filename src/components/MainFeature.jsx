...
const MainFeature = ({ tasks, onAddTask, onToggleTask, onDeleteTask }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingTask) {
        // For editing, we update the existing task
        await TaskService.updateTask(editingTask.id, taskToAdd);
        onDeleteTask(editingTask.id); // Remove the old task from the state
        setEditingTask(null);
                  ...
                  <div className="py-1">
                    {["all", "completed", "pending", "high", "medium", "low"].map((option) => (
                      <button
                        ...
                        }} className={`w-full text-left px-4 py-2 text-sm hover:bg-surface-100 dark:hover:bg-surface-700 ${filter === option
              ...
              <motion.div
                ...
                <div className="flex items-start gap-3">
                  <button
                    ...
                    } className={`mt-1 flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${task.isCompleted
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <h3 className={`text-lg font-medium ${task.isCompleted
                      ...
                      <div className="flex items-center gap-1 text-xs font-medium">
                        <span className={`px-2 py-1 rounded-full flex items-center gap-1 ${getPriorityColor(task.priority)
                    ...
                    {task.description && (
                      <p className={`mt-1 text-sm ${task.isCompleted
